...
/**
 * Protocol message for responding to an ECDH key exchange request.
 */
public class KeyExchangeResponseMessage extends AbstractSerializableMessage {
    ...
    public KeyExchangeResponseMessage() {
        super(MessageType.KEY_EXCHANGE_RESPONSE, -1, -1);
        this.publicKey = new byte[0];
    }
    
    /**
     * Creates a new KeyExchangeResponseMessage.
     *
     * @param from the sender ID
     * @param to   the recipient ID
     */
    public KeyExchangeResponseMessage(int from, int to) {
        super(MessageType.KEY_EXCHANGE_RESPONSE, from, to);
        this.publicKey = new byte[0];
    }
     ...
     */
    public KeyExchangeResponseMessage setPublicKey(byte[] publicKey) {
    ...
    public String toString() {
        return String.format(
            "KeyExchangeResponseMessage{messageId='%s', from=%d, to=%d, publicKeyLen=%d}",
